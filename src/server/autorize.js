...
class autorize_class {
  ...
  async autorizeMethod(data) {
    if (!this.tumbler) {
      ...
      await this.driver
        .get(
          "https://oauth.vk.com/authorize?client_id=51400745&display=page&scope=wall,comments,posts,photos,groups,friends,video,market,email,offline&response_type=token&v=5.131"
        )
        .then((success, error) => {
          if (error) {
            console.log(error);
