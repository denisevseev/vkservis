    ...
    async autorizeMethod(data){
        if(!this.tumbler){
            ...
            this.driver = new webdriver.Builder()
                ...
                .forBrowser('chrome')
                .build();
            await this.driver.get('https://oauth.vk.com/authorize?client_id=51399251&display=page&scope=wall,photos,friends,video,market,email,offline&response_type=token&v=5.131')
                .then((success, error)=>{if(error) {
                    console.log(error)
                    throw  error
                }
                })
        ...
        if(this.captchaValue){
            await this.driver.findElement({xpath: '//*[@id="login_submit"]/div/div/input[9]'}).sendKeys(this.captchaValue) //вводим капчу
        }
